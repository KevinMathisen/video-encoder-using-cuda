...
__host__ void gpu_init(struct c63_common *cm)
{
  ...
  CUDA_CHECK(cudaMallocManaged((void **) &d_in_org_Y, mem_size_y));
  CUDA_CHECK(cudaMallocManaged((void **) &d_in_org_U, mem_size_uv));
  CUDA_CHECK(cudaMallocManaged((void **) &d_in_org_V, mem_size_uv));

  CUDA_CHECK(cudaMallocManaged((void **) &d_in_ref_Y, mem_size_y));
  CUDA_CHECK(cudaMallocManaged((void **) &d_in_ref_U, mem_size_uv));
  CUDA_CHECK(cudaMallocManaged((void **) &d_in_ref_V, mem_size_uv));
