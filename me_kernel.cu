 ...
 */
__device__ __forceinline__ int sad_block_8x8_device(const uint8_t share_orig[8][32], 
    const uint8_t share_ref[40][64], int ref_x, int ref_y)
{
    int u, v;
    int result = 0;
...
struct macroblock *d_mbs, int range, int w, int h, int mb_cols, int mb_rows)
{
    ...
    __shared__ uint8_t share_orig[8][32]; // Use 32 bytes/row to reduce chance of bank conflict
    __shared__ uint8_t share_ref[40][64]; // Use 32x2 bytes/row to reduce chance of bank conflict
